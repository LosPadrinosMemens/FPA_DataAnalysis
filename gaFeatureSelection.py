...
def bestIndividual(hof, X, y):
    ...
    for individual in hof:
        # Compare the first element of the fitness.values tuple
        if individual.fitness.values[0] > maxAccurcy:
            maxAccurcy = individual.fitness.values[0]
            _individual = individual

    # Ensure _individual is not None before proceeding
    if _individual is not None:
        _individualHeader = [list(X)[i] for i in range(
            len(_individual)) if _individual[i] == 1]

    return maxAccurcy, _individual, _individualHeader
...
if __name__ == '__main__':
    ...
    clf = LogisticRegression(solver='newton-cholesky', max_iter=200)
